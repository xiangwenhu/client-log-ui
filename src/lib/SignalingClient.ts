...
export default class SignalingClient {
     ...
     * @returns {Promise}
     */
    login(account: string | number | null = null, token = '_no_need_token', reconnect_count = 10, reconnect_time = 30) {
    ...
    setUserAttr(name: string, value: any) {
        ...
        if (!name || !value) {
            return Promise.reject(new CodeError({
    ...
    queryUserList() {
        return this.invoke('io.agora.signal.channel_query_userlist', { name: this.channelName })
    }
    ...
    _isEmptyMessage(message: string | any) {
        if (message == null || ((typeof message === 'string' || message instanceof String) && message.trim() === '')) {
            return true
        }
        return false
    }
